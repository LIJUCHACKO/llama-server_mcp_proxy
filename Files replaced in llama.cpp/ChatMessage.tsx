...
export default function ChatMessage({
  ...
  const { pairs, isThinking } = useMemo(() => {
    if (msg.content === null || msg.role !== 'assistant') {
      return {
        pairs: [{ content: msg.content, thought: null }],
        isThinking: false,
      };
    }
    ...
    const pairs: { thought: string | null; content: string }[] = [];
    const Content = msg.content;
    //console.log(msg.content);
    let isThinking = false;
    let index = 0;
    const ContentSplit = Content.split(REGEX_THINK_OPEN);
    if (ContentSplit[index]) {
      pairs.push({ thought: null, content: ContentSplit[0] });
    }
    index++;
    let ContentPart = ContentSplit[index];

    while (ContentPart !== undefined) {
      const secondSplit = ContentPart.split(REGEX_THINK_CLOSE, 2);
      const thought = secondSplit[0];
      const content = secondSplit[1] ?? '';
      pairs.push({ thought, content });
      isThinking = true;
      ...
      index++;
      ContentPart = ContentSplit[index];
    }
    //console.log(pairs);
    return { pairs, isThinking };
  }, [msg]);
    ...
    >
      ...
      >
        ...
        >
          ...
          {editingContent === null && (
            <>
              {pairs.length === 1 && pairs[0].content === null ? (
                <>
